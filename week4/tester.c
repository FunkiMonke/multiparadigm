...
//implementation of ROI, makes a new matrix to represent inbound and outbound flights, runs dijkstras algorithm and finds shortest path based on results
void roi()
{
    //make nodes of the cities, not needed but makes it nice to read
    int dundee = 0;
    int edin = 1;
    int paris = 2;
    int london = 3;
    ...
    //make matrix, set all to 0
    AdjacencyMatrix* flight = NULL;
    flight = createAdjacencyMatrix(0);

    //make tables and initialise
    DijkstraTable mTe;
    DijkstraTable eTm;

    for (int i = 0; i < NUMBER_OF_VERTICES; i++)
    {
        mTe.table[i].visited = false;
        eTm.table[i].visited = false;

        mTe.table[i].distance = VERY_LARGE_NUMBER;
        eTm.table[i].distance = VERY_LARGE_NUMBER;

        mTe.table[i].predecessor = -1;
        eTm.table[i].predecessor = -1;
    }

    //make path array, set all to -1
    ...
    addEdge(flight, dundee, edin, 30);
    addEdge(flight, edin, paris, 115);
    addEdge(flight, paris, madrid, 115);      //outbound
    addEdge(flight, edin, london, 75);
    addEdge(flight, london, madrid, 115);
    addEdge(flight, dundee, london, 85);

    addEdge(flight, edin, dundee, 45);
    addEdge(flight, paris, edin, 110);
    addEdge(flight, paris, london, 120);      //inbound
    addEdge(flight, london, edin, 75);
    addEdge(flight, madrid, london, 105);

    //calculate madrid to edin
    runDijsktraAlgorithm(flight, &mTe, madrid);
    getShortestPathFrom(&mTe, madrid, edin, path);

    printf("\nFrom Madrid to Edinburgh: ");
    for (int i=0; i<NUMBER_OF_VERTICES; i++)
    {
        printf("[%d]", path[i]);
    }
    ...
    //calculate edin to madrid
    printf("From Edinburgh to Madrid: ");
    runDijsktraAlgorithm(flight, &eTm, edin);
    getShortestPathFrom(&eTm, edin, madrid, path);
