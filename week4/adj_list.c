...
int addEdgeToAdjacencyList(AdjacencyList *pList, int src, int dest, int weight)
{
    ...
    ListNode *n = NULL;
    n = (ListNode*)myMalloc(sizeof(ListNode));
    ...
    //add to list, adding to head
    n->next = pList->adjacencyList[src];
    pList->adjacencyList[src] = n;
...
int populateAdjacencyMatrixFromAdjacencyList(AdjacencyMatrix *pMatrix, AdjacencyList *pList)
{
    //allocate memory
    ListNode *iter = NULL;
    iter = (ListNode*)myMalloc(sizeof(ListNode));
    ...
    {
        //continue for all elements in row / all nodes it shares an edge with
        iter = pList->adjacencyList[i];
        while (iter != NULL)
        {   //add to matrix, then go to next element
            pMatrix->matrix[i][iter->destNode] = iter->weight;
            iter = iter->next;
        }
